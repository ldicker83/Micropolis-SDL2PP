...
namespace
{
    ...
    void resetButtonsToNormal(std::array<MiniMapWindow::ButtonMeta, 14>& buttons)
    {
        ...
        {
            button.state = MiniMapWindow::ButtonState::Normal;
        }
    }
};
...
void MiniMapWindow::drawUI()
{
    ...
    {
        SDL_RenderTexture(mRenderer, mButtonTextures.texture, &mButtonUV[i + (static_cast<int>(mButtons[i].state) * static_cast<size_t>(arraySize))], &mButtons[i].rect);
    }
...
void MiniMapWindow::handleNoUiButtonSelected()
{
    ...
    {
        mButtons[0].state = ButtonState::Pressed;
